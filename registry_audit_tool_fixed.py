...
class RegistryAuditor:
    ...
    def audit_registry(self):
        ...
        for category, indicators in self.current_registry.items():
            ...
            # Map current categories to recovery plan categories (normalize case)
            category_upper = category.upper()
            expected_count = self.recovery_plan_requirements.get(category_upper, 0)
            status = "✅ COMPLIANT" if category_count == expected_count else "❌ MISMATCH"
            
            # Special handling for categories not in recovery plan
            if category_upper not in self.recovery_plan_requirements:
            ...
            if category_count != expected_count or expected_count == 0:
                audit_results['discrepancies'].append({
                    'category': category,
                    'current': category_count,
                    'required': expected_count,
                    'difference': category_count - expected_count,
                    'note': 'Extra category not in recovery plan' if expected_count == 0 else 'Count mismatch'
                })
        
        # Check for missing categories from recovery plan
        current_categories_upper = {cat.upper() for cat in self.current_registry.keys()}
        missing_categories = set(self.recovery_plan_requirements.keys()) - current_categories_upper
    ...
    def generate_corrected_registry(self, audit_results):
        ...
        for category, required_count in self.recovery_plan_requirements.items():
            # Find matching category in current registry (case-insensitive)
            current_indicators = []
            for curr_cat, indicators in self.current_registry.items():
                if curr_cat.upper() == category.upper():
