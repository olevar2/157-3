...
class KnowSureThing(IndicatorBase):
    ...
    def __init__(
        ...
        if (
            any(p <= 0 for p in self.roc_periods + self.sma_periods)
    ...
    def calculate(self, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            min_required = (
                max(self.roc_periods) + max(self.sma_periods) + self.signal_period
            )
            ...
            return {
                ...
                "signals": signals,
                "metrics": metrics,
                "interpretation": self._interpret_signals(
                    kst[-1], signal_line[-1], signals[-1]
                ),
            }
    ...
    def _calculate_kst(self, close: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(len(close)):
            if (
                not np.isnan(smoothed_roc1[i])
                and not np.isnan(smoothed_roc2[i])
                and not np.isnan(smoothed_roc3[i])
                and not np.isnan(smoothed_roc4[i])
            ):

                kst[i] = (
                    smoothed_roc1[i] * weights[0]
    ...
    def _calculate_metrics(self, kst: np.ndarray, signal_line: np.ndarray) -> Dict:
        ...
        for i in range(1, min(len(valid_kst), len(valid_signal))):
            if (
                valid_kst[i - 1] <= valid_signal[i - 1]
                and valid_kst[i] > valid_signal[i]
            ) or (
                valid_kst[i - 1] >= valid_signal[i - 1]
                and valid_kst[i] < valid_signal[i]
            ):
        ...
        return {
            ...
            "current_signal": (
                signal_line[-1] if not np.isnan(signal_line[-1]) else None
            ),
    ...
    def _interpret_signals(
        self, current_kst: float, current_signal: float, current_signal_val: float
    ) -> str:
...
def create_know_sure_thing(
    roc_periods: Tuple[int, int, int, int] = (10, 15, 20, 30), **kwargs
) -> KnowSureThing:
