...
class GannAnglesCalculator(IndicatorBase):
    ...
    Features:
    - Classic Gann angles (1x1, 2x1, 1x2, 4x1, 1x4, 8x1, 1x8)
    - Dynamic angle calculation from swing points
    ...
    def calculate(self, data: pd.DataFrame) -> IndicatorResult:
        ...
        try:
            ...
            return IndicatorResult(                timestamp=data.index[-1],
                indicator_name="GannAnglesCalculator",
                indicator_type=IndicatorType.GANN,
                timeframe=TimeFrame.D1,
                value=gann_angles,
            ...
            return IndicatorResult(                timestamp=data.index[-1] if len(data) > 0 else pd.Timestamp.now(),
                indicator_name="GannAnglesCalculator",
                indicator_type=IndicatorType.GANN,
                timeframe=TimeFrame.D1,
                value=[],
    ...
    def _find_swing_points(self, data: pd.DataFrame) -> Tuple[List[Tuple[int, float]], List[Tuple[int, float]]]:
    ...
    def _calculate_angles_from_point(self, data: pd.DataFrame, 
                                   start_point: Tuple[int, float], 
                                   direction: str) -> List[GannAngle]:
        ...
        for ratio_name, angle_degrees in self.gann_ratios.items():
            ...
            if 'x' in ratio_name:
                ...
                angle = GannAngle(
                    ratio=ratio_name,
                    angle_degrees=angle_degrees if direction == 'up' else 180 - angle_degrees,
                    slope=slope,
    ...
    def _analyze_current_position(self, data: pd.DataFrame, 
                                gann_angles: List[GannAngle]) -> Dict[str, Any]:
        ...
        support_angles = [a for a in gann_angles if a.is_support and a.current_value < current_price]
        resistance_angles = [a for a in gann_angles if not a.is_support and a.current_value > current_price]
        
        nearest_support = max(support_angles, key=lambda x: x.current_value) if support_angles else None
        nearest_resistance = min(resistance_angles, key=lambda x: x.current_value) if resistance_angles else None
        
        # Find strongest angle influence
        strongest_angle = max(gann_angles, key=lambda x: x.strength) if gann_angles else None
