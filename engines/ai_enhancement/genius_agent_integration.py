...
Connects all 9 genius agents to the 129 indicators through the enhanced adaptive coordinator
"""
...
class GeniusAgentIntegration:
    ...
    def __init__(self):
        ...
        self.logger.info("Genius Agent Integration initialized with 129 indicators")
    ...
    def make_trading_decision(self, symbol: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(market_data, list):
            ...
            market_info = self._extract_market_info(market_data)
        self.logger.info(f"Making trading decision for {market_info.get('symbol', 'unknown')}")
        ...
        try:
            ...
            formatted_decision = {
                ...
                'position_size': trading_decision.get('position_sizing', {}).get('recommended_position_size'),
    ...
    def generate_trading_signal(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        final_action = action_map.get(
            trading_decision.get('final_recommendation', 'HOLD').upper(),
            'HOLD'
        )
        ...
        signal = {
            ...
            'stop_loss': trading_decision.get('stop_loss_level'),
            'take_profit': trading_decision.get('take_profit_level'),
            'position_size_pct': trading_decision.get('position_sizing', {}).get('recommended_size_pct', 1.0),
    ...
    def execute_full_analysis(self, market_data: Dict) -> Dict[str, Any]:
        """Execute complete analysis using all agents and 129 indicators"""
        ...
        try:
            if hasattr(self.coordinator, 'calculate_multi_agent_signals'):
                multi_agent_signals = self.coordinator.calculate_multi_agent_signals(market_data)
            else:
        ...
        for agent_name, agent_interface in self.genius_agents.items():
            try:
                agent_signals = multi_agent_signals.get('agent_signals', {}).get(agent_name, {})
        ...
        return {
            ...
            'analysis_metadata': {
                'total_indicators_analyzed': multi_agent_signals.get('total_indicators_used', 129),
                'analysis_duration_seconds': analysis_duration,
                'timestamp': analysis_start.isoformat(),
                'market_regime': multi_agent_signals.get('market_regime', 'normal'),
                'participating_agents': len([a for a in agent_analyses.values() 
                                           if a.get('status') != 'failed'])
            }
        }
    
    def _generate_trading_decision(self, multi_agent_signals: Dict, 
                                 agent_analyses: Dict, market_data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'take_profit_level': self._calculate_take_profit(agent_analyses, market_data),
    ...
    def _calculate_consensus_metrics(self, weighted_insights: Dict) -> Dict[str, float]:
        ...
        buy_weight = sum(insight['weighted_score'] for insight in weighted_insights.values()
                        if insight['recommendation'] in ['BUY', 'STRONG_BUY'])
        
        sell_weight = sum(insight['weighted_score'] for insight in weighted_insights.values()
                         if insight['recommendation'] in ['SELL', 'STRONG_SELL'])
        
        total_weight = sum(insight['weighted_score'] for insight in weighted_insights.values())
    ...
    def _assess_overall_risk(self, agent_analyses: Dict, market_data: Dict) -> Dict[str, Any]:
        ...
        pattern_risk = agent_analyses.get('pattern_master', {}).get('volatility_assessment', 0.5)
        
        # Market microstructure risk
        microstructure_risk = agent_analyses.get('market_microstructure_genius', {}).get('liquidity_risk', 0.5)
    ...
    def _calculate_position_sizing(self, master_decision: Dict, 
                                 risk_assessment: Dict, consensus_metrics: Dict) -> Dict[str, float]:
        ...
        recommended_size = base_size * confidence_multiplier * consensus_multiplier * risk_multiplier
          # Apply safety limits
        max_position_size = 0.05  # Maximum 5% of account
        min_position_size = 0.001  # Minimum 0.1% of account
        
        recommended_size = max(min_position_size, min(max_position_size, recommended_size))
    ...
    def _calculate_stop_loss(self, overall_risk: float, market_data: Dict) -> Dict[str, float]:
        ...
        volatility_multiplier = max(0.5, min(2.0, volatility / 0.02))  # Scale around 2% base
        ...
        final_stop_percentage = base_stop_percentage * volatility_multiplier * condition_multiplier
    ...
    def _get_position_sizing_rationale(self, confidence_multiplier: float, 
                                      consensus_multiplier: float, risk_multiplier: float) -> str:
        ...
        base_rationale = f"{rationale_parts[0]} with {rationale_parts[1]} {rationale_parts[2]}"
        
        # Add sizing recommendation
        total_multiplier = confidence_multiplier * consensus_multiplier * risk_multiplier
    ...
    def get_agent_status_report(self) -> Dict[str, Any]:
        ...
        status_report = {
            'timestamp': datetime.now().isoformat(),
            'total_indicators_available': 129,
            'agent_status': {},
            'coordinator_status': {
                'active': True,
        ...
        healthy_agents = sum(1 for status in status_report['agent_status'].values()
    ...
    def _extract_key_insights(self, agent_analyses: Dict) -> List[str]:
        ...
        for agent_name, analysis in agent_analyses.items():
            ...
            if isinstance(signal, dict):
                ...
                if action in ['BUY', 'SELL'] and strength > 0.7:
                    insights.append(f"{agent_name}: Strong {action} signal (strength: {strength:.1%})")
            ...
            if isinstance(volatility, (int, float)):
                if volatility > 0.05:
                    insights.append(f"{agent_name}: High volatility detected ({volatility:.1%})")
        ...
        if len(agent_analyses) > 1:
            ...
            buy_signals = sum(1 for analysis in agent_analyses.values() 
                            if isinstance(analysis, dict) and 
                            analysis.get('signal', {}).get('action') == 'BUY')
            sell_signals = sum(1 for analysis in agent_analyses.values() 
                             if isinstance(analysis, dict) and 
                             analysis.get('signal', {}).get('action') == 'SELL')
    ...
    def _calculate_take_profit(self, agent_analyses: Dict, market_data: Dict) -> Dict[str, float]:
        ...
        return {
            ...
            'risk_reward_ratio': avg_risk_reward,            'target_rationale': f"Based on {len(profit_targets)} agent signals with "
    ...
    def _estimate_trade_validity(self, agent_analyses: Dict) -> str:
        ...
        for agent_name, analysis in agent_analyses.items():
            ...
            strength = analysis.get('signal', {}).get('strength', 0.5) if isinstance(analysis.get('signal'), dict) else 0.5
            confidence = analysis.get('confidence', 0.5)
    ...
    def _calculate_comprehensive_risk(self, market_data: Dict, 
                                    indicator_values: Dict) -> Dict[str, Any]:
    ...
    def _extract_market_info(self, market_data) -> Dict[str, Any]:
        ...
        if isinstance(market_data, list):
            if len(market_data) == 0:
                return {'price': 100.0, 'close': 100.0, 'symbol': 'unknown', 'timeframe': '1h', 'volatility': 0.02, 'market_condition': 'normal'}
    ...
    def _generate_fallback_signals(self, market_data: Dict) -> Dict[str, Any]:
        ...
        fallback_signals = {
            'agent_signals': {
                'risk_genius': {'signal': 'neutral', 'strength': 0.5, 'confidence': 0.6},
                'session_expert': {'signal': 'active', 'strength': 0.7, 'confidence': 0.7},
                'pattern_master': {'signal': 'scanning', 'strength': 0.6, 'confidence': 0.6},
                'execution_expert': {'signal': 'ready', 'strength': 0.8, 'confidence': 0.8},
                'pair_specialist': {'signal': 'analyzing', 'strength': 0.6, 'confidence': 0.6},
                'decision_master': {'signal': 'evaluating', 'strength': 0.7, 'confidence': 0.7},
                'market_microstructure_genius': {'signal': 'monitoring', 'strength': 0.5, 'confidence': 0.5},
                'multi_timeframe_genius': {'signal': 'coordinating', 'strength': 0.6, 'confidence': 0.6},
                'adaptive_optimization_genius': {'signal': 'optimizing', 'strength': 0.5, 'confidence': 0.5}
            },
            'total_indicators_used': 129,
...
class ExecutionExpertInterface(BaseAgentInterface):
    ...
    def execute_analysis(self, market_data: Dict, agent_signals: Dict) -> Dict[str, Any]:
        ...
        if isinstance(market_data, list) and len(market_data) > 0:
            ...
            close_price = latest_data.get('close', 100.0) if isinstance(latest_data, dict) else 100.0
        elif isinstance(market_data, dict):
