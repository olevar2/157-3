...
class AdaptiveIndicatorBridge:
    ...
    def __init__(self):
        ...
        self.smart_cache = {}  # Phase 4C performance optimization
        self.calculation_pool = None  # For parallel processing
    ...
    def _build_comprehensive_agent_mapping(self) -> Dict[GeniusAgentType, Dict]:
        ...
        - PATTERN_MASTER: 52+ indicators (CLEANED: removed 8 Gann aliases)
        ...
        return {
            ...
            GeniusAgentType.PATTERN_MASTER: {
                "primary_indicators": [
                    ...
                    "wave_type",
                    "wave_structure",
                    "wave_point",  # Real Gann indicators (7) - CLEANED: Primary names only, no duplicates
                    "gann_angles_calculator",  # Real Gann angle calculations
                    "gann_grid",  # Real Gann grid analysis
                    "gann_pattern_detector",  # Real Gann pattern recognition
                    "gann_fan_lines",  # Real Gann fan line analysis
                    "gann_square_of_nine",  # Real Square of Nine
                    "gann_time_cycles",  # Real time cycle analysis
                    "price_time_relationships",  # Real price-time analysis
                ...
                "secondary_indicators": [
                    ...
                    "momentum_divergence_scanner",
                ],
                "adaptive_features": [
                    "pattern_confidence",
                    "pattern_completion",
                    "fractal_pattern_strength",
                    "harmonic_validation",
                    "elliott_wave_count",
                ],
            },
            ...
            GeniusAgentType.SESSION_EXPERT: {
                "primary_indicators": [
                    ...
                    "phase_analysis",  # Elliott Wave & Gann (5)
                    "wave_type",
                    "wave_structure",
                    "gann_fan_lines",  # Updated to primary name (was gann_angle)
                    "time_cycle",
                    "pivot_type",
                ],
                "secondary_indicators": [
            ...
            GeniusAgentType.AI_MODEL_COORDINATOR: {
                ...
                "secondary_indicators": ["pattern_indicators", "momentum_indicators"],
                "adaptive_features": [
                    "model_confidence",
                    "prediction_accuracy",
                    "ensemble_weighting",
            ...
            GeniusAgentType.SENTIMENT_INTEGRATION_GENIUS: {
                ...
                "secondary_indicators": ["volume_indicators", "volatility_indicators"],
                "adaptive_features": [
                    "sentiment_strength",
            ...
            GeniusAgentType.PAIR_SPECIALIST: {
                ...
                "secondary_indicators": ["volume_indicators", "trend_indicators"],
                "adaptive_features": [
            ...
            GeniusAgentType.MARKET_MICROSTRUCTURE_GENIUS: {
                "primary_indicators": [
                    ...
                    "momentum_divergence_scanner",
                    "composite_signal",
                    "advanced_ml_engine",  # Momentum indicators relevant to microstructure (6)
                    "correlation_matrix",
                    "dynamic_correlation",
                    "relative_momentum",
                    "stochastic_oscillator",
                    "moving_average_convergence_divergence",
                    "relative_strength_index",
                    # Gann time-price analysis for microstructure (3)
                    "gann_time_cycles",  # For timing institutional moves
                    "price_time_relationships",  # For price-time correlation in microstructure
                    "gann_angles_calculator",  # For support/resistance in order flow
                ],
                "secondary_indicators": ["fractal_indicators", "pattern_indicators"],
                "adaptive_features": [
    ...
    async def get_agent_indicators_async(
        ...
        try:
            self.logger.info(
                f"Processing indicators for {agent_type.value}"
            )  # Get comprehensive indicator package
            ...
            self.performance_monitor.log_metric(
                "optimization_score", indicator_package.optimization_score
            )

            self.logger.info(
                f"Successfully processed {len(indicators)} indicators for {agent_type.value}"
            )
            return indicators
    ...
    async def prepare_indicators_for_genius(
        self,
        market_data: Dict[str, Any],
        agent_type: GeniusAgentType,
        context: Optional[Dict] = None,
        ...
        try:
            ...
            primary_indicators = await self._calculate_indicator_set(
                agent_config["primary_indicators"], market_data, market_regime
            )
            ...
            optimization_score = self._calculate_optimization_score(
                optimized_indicators, agent_type, market_regime
            )
    ...
    async def _calculate_indicator_set(
        self,
    ...
    async def _optimize_for_agent(
        ...
        if agent_type == GeniusAgentType.RISK_GENIUS:
            ...
            if "correlation_analysis" in indicators:
                indicators["portfolio_correlation"] = (
                    await self._calculate_portfolio_correlation(
            ...
            indicators["composite_risk_score"] = self._calculate_composite_risk(
                indicators
            )
            ...
            for pattern_key in ["harmonic_patterns", "elliott_wave", "chart_patterns"]:
                if pattern_key in indicators:
                    indicators[f"{pattern_key}_ml_confidence"] = (
                        await self._calculate_ml_confidence(
                            indicators[pattern_key], market_regime
                        )
                    )
    ...
    async def get_adaptive_indicator_recommendations(
        self, agent_type: GeniusAgentType, market_conditions: Dict[str, Any]
    ) -> List[str]:
        ...
        if volatility_level == "high" and agent_type == GeniusAgentType.RISK_GENIUS:
            recommendations.extend(
                ["garch_volatility", "var_stress_test", "tail_risk_indicator"]
            )
        if trend_strength == "strong" and agent_type == GeniusAgentType.PATTERN_MASTER:
            recommendations.extend(
                ["trend_continuation_patterns", "momentum_divergence"]
            )
    ...
    async def _detect_market_regime(self, market_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if (
                not isinstance(close_prices, (list, np.ndarray))
                or len(close_prices) < 10
            ):
                return "neutral"

            # Calculate recent volatility
            recent_prices = (
                close_prices[-20:] if len(close_prices) >= 20 else close_prices
            )
    ...
    async def _calculate_single_indicator(
        ...
        try:
            ...
            indicator_instance = await self._instantiate_indicator(
                indicator_name, {}, adaptive_params
            )
            if indicator_instance is None:
                return self._get_default_value(indicator_name)

            # Calculate the indicator
            result = await self._try_calculate_indicator(
                indicator_instance, formatted_data, adaptive_params, indicator_name
            )
    ...
    async def _calculate_adaptive_indicators(
        self,
        indicator_names: List[str],
        market_data: Dict[str, Any],
        market_regime: str,
        ...
        try:
            ...
            for indicator_name in selected_indicators:
                try:
                    ...
                    print(
                        f"DEBUG: Error calculating adaptive indicator {indicator_name}: {str(e)}"
                    )
                    continue
    ...
    def _calculate_optimization_score(
        ...
        try:
            ...
            base_score = min(
                indicator_count / 20.0, 1.0
            )  # Normalize to max 20 indicators

            # Agent-specific bonus
            agent_config = self.agent_indicator_mapping.get(agent_type, {})
            primary_indicators = agent_config.get("primary_indicators", [])
            primary_count = sum(
                1 for name in indicators.keys() if name in primary_indicators
            )
            ...
            total_score = (
                base_score + agent_bonus + regime_bonus + quality_score
            ) * 10.0
                ...
                if key in market_data:
                    ...
                    if key == "volume":
                        ...
                        if isinstance(base_price, (list, np.ndarray)):
                            base_price = (
                                base_price[-1] if len(base_price) > 0 else 1.0500
                            )
    ...
    async def _instantiate_indicator(
        self, indicator_name: str, config: Dict, adaptive_params: Dict
    ) -> Any:
        ...
        try:
            ...
            if "Signal" in indicator_name:
                ...
                try:
                    ...
                    indicator_class = get_indicator(indicator_name)
                    return indicator_class(
                        price=1.05, volume=1000, timestamp=datetime.now()
                    )
                except:
                    print(f"DEBUG: Could not instantiate {indicator_name}: {error_msg}")
                    return None
            ...
            try:
                ...
                try:  # Try with period parameter
                    ...
                    try:
                        ...
                        print(
                            f"DEBUG: Could not instantiate {indicator_name}: {error_msg}"
                        )
                        return None

    async def _try_calculate_indicator(
        self,
        indicator_instance,
        formatted_data: Dict,
            ...
            if "close_list" in data_formats:
                try:
                    df_data = {
                        ...
                        "open": data_formats.get(
                            "open_list", data_formats["close_list"]
                        ),
                        "high": data_formats.get(
                            "high_list", data_formats["close_list"]
                        ),
                        "low": data_formats.get("low_list", data_formats["close_list"]),
                        "volume": data_formats.get(
                            "volume_list", [1000] * len(data_formats["close_list"])
                        ),
                    }
        ...
        try:
            ...
            if "statistical" in indicator_name or any(
                stat_name in indicator_name
                for stat_name in [
                    "cointegration",
                    "linear_regression",
                    "r_squared",
                ...
                if "close_list" in data_formats and len(data_formats["close_list"]) > 0:
                    result = indicator_instance.calculate(
                        data_formats["close_list"], **adaptive_params
                    )
                else:
                    result = indicator_instance.calculate(
                        [1.0, 1.1, 1.05, 1.08, 1.12] * 20
                    )  # Ensure enough data
            else:
            ...
            try:
                ...
                # Method 5: Try with OHLCV parameters
                    ...
                    result = indicator_instance.calculate(
                        high=data_formats.get(
                            "high_list", data_formats.get("close_list", [])
                        ),
                        low=data_formats.get(
                            "low_list", data_formats.get("close_list", [])
                        ),
                ...
                if hasattr(result, "__await__"):
                    result = await result
                    print(
                        f"DEBUG: {indicator_name} calculated successfully (async, retry)"
                    )
                else:
                    print(
                        f"DEBUG: {indicator_name} calculated successfully (sync, retry)"
                    )
                ...
                try:
                    if hasattr(indicator_instance, "calculate"):
                        if (
                            "close_list" in data_formats
                            and len(data_formats["close_list"]) > 0
                        ):
                            ...
                            if "volume" in str(e2):
                                result = indicator_instance.calculate(
                                    ...
                                    volume=data_formats.get(
                                        "volume_list",
                                        [1000] * len(data_formats["close_list"]),
                                    ),
                                )
                            else:
                                result = indicator_instance.calculate(
                            ...
                            if hasattr(result, "__await__"):
                                result = await result
                            print(
                                f"DEBUG: {indicator_name} calculated with fallback method"
                            )
                        else:
                            print(
                                f"DEBUG: All calculation methods failed for {indicator_name}"
                            )
                            return None
                    else:
                    ...
                    print(
                        f"DEBUG: Final fallback failed for {indicator_name}: {str(e3)}"
                    )
                    return None

        except Exception as e:
            print(
                f"DEBUG: Exception in _try_calculate_indicator for {indicator_name}: {str(e)}"
            )
            return None
    ...
    async def get_comprehensive_indicator_package(
        self,
        agent_type: GeniusAgentType,
        market_data: Dict[str, Any],
        ...
        try:
            ...
            all_indicators = (
                agent_config.get("primary_indicators", [])
            ...
            for indicator in all_indicators:
                ...
                if isinstance(indicator, str) and indicator not in seen:
                    ...
                    print(
                        f"DEBUG: Skipping non-string indicator: {type(indicator)} - {indicator}"
                    )
            ...
            print(
                f"DEBUG: Agent {agent_type.value} - Available: {len(unique_indicators)}, Selected: {len(selected_indicators)}"
            )
            ...
            for indicator_name in selected_indicators:
                try:
                    ...
                    if indicator_name in self.indicator_registry:
                        ...
                        print(
                            f"DEBUG: Indicator {indicator_name} not found in registry"
                        )

                except TypeError as e:
                    if "not callable" in str(e):
                        print(
                            f"WARNING: Skipping indicator {indicator_name}: not callable"
                        )
                    else:
                        print(
                            f"DEBUG: TypeError calculating {indicator_name}: {str(e)}"
                        )
                    continue
                except Exception as e:
                    print(f"DEBUG: Error calculating {indicator_name}: {str(e)}")
            ...
            optimization_score = (
                min(len(calculated_indicators) / max(len(selected_indicators), 1), 1.0)
                * 10.0
            )

            print(
                f"DEBUG: Agent {agent_type.value} - Calculated: {len(calculated_indicators)} indicators in {calculation_time:.1f}ms"
            )
    ...
    async def _calculate_performance_scores(
        ...
        for indicator_name in indicator_names:
            if indicator_name in self.indicator_registry:
                ...
                regime_score = self._get_regime_adaptation_score(
                    indicator_name, market_regime
                )
                ...
                # Combine scores
                final_score = (
                    base_score * agent_affinity * regime_score * category_score
                )
                scores[indicator_name] = final_score

        return scores

    def _get_regime_adaptation_score(
        self, indicator_name: str, market_regime: str
    ) -> float:
    ...
    def _get_category_diversity_score(
    ...
    def _get_fallback_indicators(
        self, agent_type: GeniusAgentType, market_data: Dict
    ) -> IndicatorPackage:
    ...
    async def _calculate_portfolio_correlation(
        self, correlation_data: Any, assets: List[str]
    ) -> Dict:
    ...
    async def _calculate_ml_confidence(
        self, pattern_data: Any, market_regime: str
    ) -> float:
