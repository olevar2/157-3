...
BlockTradeSignal volume analysis indicator for institutional flow detection and volume momentum analysis.
...
class BlockTradeSignal(StandardIndicatorInterface):
    """BlockTradeSignal for volume analysis"""
    ...
    def calculate(self, data: Union[pd.DataFrame, pd.Series]) -> pd.DataFrame:
        ...
        result = pd.DataFrame({
            'block_trade_signal': result_value,
            'signals': signals
        }, index=index)
        
        self._last_calculation = {"block_trade_signal": result_value, "signals": signals}
        return result
    ...
    def get_metadata(self) -> IndicatorMetadata:
        return IndicatorMetadata(
            name="BlockTradeSignal", category=self.CATEGORY,
            description="BlockTradeSignal - Volume analysis indicator",
    ...
    def get_config(self) -> Dict[str, Any]:
        return {"name": "BlockTradeSignal", "category": self.CATEGORY, 
