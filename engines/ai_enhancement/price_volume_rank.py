...
PriceVolumeRank volume analysis indicator for institutional flow detection and volume momentum analysis.
...
class PriceVolumeRank(StandardIndicatorInterface):
    """PriceVolumeRank for volume analysis"""
    ...
    def calculate(self, data: Union[pd.DataFrame, pd.Series]) -> pd.DataFrame:
        ...
        result = pd.DataFrame({
            'price_volume_rank': result_value,
            'signals': signals
        }, index=index)
        
        self._last_calculation = {"price_volume_rank": result_value, "signals": signals}
        return result
    ...
    def get_metadata(self) -> IndicatorMetadata:
        return IndicatorMetadata(
            name="PriceVolumeRank", category=self.CATEGORY,
            description="PriceVolumeRank - Volume analysis indicator",
    ...
    def get_config(self) -> Dict[str, Any]:
        return {"name": "PriceVolumeRank", "category": self.CATEGORY, 
                "parameters": self.parameters, "version": self.VERSION}

def get_indicator_class():
    return PriceVolumeRank
