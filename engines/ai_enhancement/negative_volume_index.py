...
NegativeVolumeIndex volume analysis indicator for institutional flow detection and volume momentum analysis.
...
class NegativeVolumeIndex(StandardIndicatorInterface):
    """NegativeVolumeIndex for volume analysis"""
    ...
    def calculate(self, data: Union[pd.DataFrame, pd.Series]) -> pd.DataFrame:
        ...
        result = pd.DataFrame({
            'negative_volume_index': result_value,
            'signals': signals
        }, index=index)
        
        self._last_calculation = {"negative_volume_index": result_value, "signals": signals}
        return result
    ...
    def get_metadata(self) -> IndicatorMetadata:
        return IndicatorMetadata(
            name="NegativeVolumeIndex", category=self.CATEGORY,
            description="NegativeVolumeIndex - Volume analysis indicator",
    ...
    def get_config(self) -> Dict[str, Any]:
        return {"name": "NegativeVolumeIndex", "category": self.CATEGORY, 
                "parameters": self.parameters, "version": self.VERSION}

def get_indicator_class():
    return NegativeVolumeIndex
