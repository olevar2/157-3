...
class AbandonedBabyPatternEngine(BasePatternEngine):
    ...
    def __init__(
        ...
        max_doji_body_ratio: float = 0.1,
        min_gap_percentage: float = 0.2,
    ):
        super().__init__(config=config)  # Pass config to BasePatternEngine
        self.min_body_ratio = (
            min_body_ratio  # Minimum body ratio for first and third candles
        )
        self.max_doji_body_ratio = (
            max_doji_body_ratio  # Maximum body ratio for doji candle
        )
        self.min_gap_percentage = (
            min_gap_percentage  # Minimum gap size as % of candle range
        )
            ...
            try:
                ...
                ohlcv_candles.append(
                    OHLCV(
                        timestamp=item[
                            "timestamp"
                        ],  # or item.get('timestamp') or item.get('date')
    ...
    def detect_abandoned_baby_pattern(
        ...
        bullish_result = self._detect_bullish_abandoned_baby(
            first_candle, doji_candle, third_candle
        )
        if bullish_result:
            return bullish_result

        # Check for bearish abandoned baby pattern
        bearish_result = self._detect_bearish_abandoned_baby(
            first_candle, doji_candle, third_candle
        )
        if bearish_result:
    ...
    def _detect_bullish_abandoned_baby(
        self, first: OHLCV, doji: OHLCV, third: OHLCV
    ) -> Optional[Dict[str, Any]]:
        ...
        second_gap_percentage = (
            (second_gap / third_range) * 100 if third_range > 0 else 0
        )

        if (
            first_gap_percentage < self.min_gap_percentage
            or second_gap_percentage < self.min_gap_percentage
        ):
            return None
        ...
        strength = self._calculate_pattern_strength(
            first_gap_percentage, second_gap_percentage, doji_quality
        )
    ...
    def _detect_bearish_abandoned_baby(
        self, first: OHLCV, doji: OHLCV, third: OHLCV
    ) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_pattern_strength(
        self,
        first_gap_percentage: float,
        second_gap_percentage: float,
        doji_quality: float,
    ) -> str:
    ...
    def generate_signal(self, pattern_data: Dict[str, Any]) -> AbandonedBabySignal:
        ...
        if pattern_data["pattern_type"] == "bullish_abandoned_baby":
            ...
            )  # 3:1 R/R ratio (stronger pattern)
