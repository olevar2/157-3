...
VPTTrendState volume analysis indicator for institutional flow detection and volume momentum analysis.
...
class VPTTrendState(StandardIndicatorInterface):
    """VPTTrendState for volume analysis"""
    ...
    def calculate(self, data: Union[pd.DataFrame, pd.Series]) -> pd.DataFrame:
        ...
        result = pd.DataFrame({
            'v_p_t_trend_state': result_value,
            'signals': signals
        }, index=index)
        
        self._last_calculation = {"v_p_t_trend_state": result_value, "signals": signals}
        return result
    ...
    def get_metadata(self) -> IndicatorMetadata:
        return IndicatorMetadata(
            name="VPTTrendState", category=self.CATEGORY,
            description="VPTTrendState - Volume analysis indicator",
    ...
    def get_config(self) -> Dict[str, Any]:
        return {"name": "VPTTrendState", "category": self.CATEGORY, 
                "parameters": self.parameters, "version": self.VERSION}

def get_indicator_class():
    return VPTTrendState
