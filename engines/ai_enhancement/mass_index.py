...
MassIndex volatility analysis indicator for risk measurement and volatility regime detection.
...
class MassIndex(StandardIndicatorInterface):
    """MassIndex for volatility analysis"""
    ...
    def calculate(self, data: Union[pd.DataFrame, pd.Series]) -> pd.DataFrame:
        ...
        result = pd.DataFrame({
            'mass_index': result_value,
            'signals': signals
        }, index=index)
        
        self._last_calculation = {"mass_index": result_value, "signals": signals}
        return result
    ...
    def get_metadata(self) -> IndicatorMetadata:
        return IndicatorMetadata(
            name="MassIndex", category=self.CATEGORY,
            description="MassIndex - Volatility analysis indicator",
    ...
    def get_config(self) -> Dict[str, Any]:
        return {"name": "MassIndex", "category": self.CATEGORY, 
