...
class MomentumIndicator(IndicatorBase):
    ...
    def __init__(self, period: int = 14, method: str = "ratio"):
        """
        ...
        for i in range(1, len(momentum)):
            ...
            # Momentum exhaustion signals
                ...
                if (
                    ...
                    and momentum[i - 1] > momentum[i]
                    and momentum[i - 1] > neutral_line
                ):
    ...
    def _calculate_metrics(self, momentum: np.ndarray) -> Dict:
        ...
        positive_momentum_pct = (
            np.sum(valid_values > neutral_line) / len(valid_values) * 100
        )
        ...
        if self.method == "ratio":
            ...
            strong_positive_pct = (
                np.sum(valid_values > mean_val + std_dev) / len(valid_values) * 100
            )
            strong_negative_pct = (
                np.sum(valid_values < mean_val - std_dev) / len(valid_values) * 100
            )
            extreme_positive_pct = (
                np.sum(valid_values > mean_val + 2 * std_dev) / len(valid_values) * 100
            )
        ...
        for i in range(1, len(valid_values) - 1):
            if (
                ...
                and valid_values[i] > valid_values[i + 1]
            ):
                peaks += 1
            elif (
                valid_values[i] < valid_values[i - 1]
...
def create_momentum_indicator(
    period: int = 14, method: str = "ratio", **kwargs
) -> MomentumIndicator:
