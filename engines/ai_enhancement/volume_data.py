...
VolumeData volume analysis indicator for institutional flow detection and volume momentum analysis.
...
class VolumeData(StandardIndicatorInterface):
    """VolumeData for volume analysis"""
    ...
    def calculate(self, data: Union[pd.DataFrame, pd.Series]) -> pd.DataFrame:
        ...
        result = pd.DataFrame({
            'volume_data': result_value,
            'signals': signals
        }, index=index)
        
        self._last_calculation = {"volume_data": result_value, "signals": signals}
        return result
    ...
    def get_metadata(self) -> IndicatorMetadata:
        return IndicatorMetadata(
            name="VolumeData", category=self.CATEGORY,
            description="VolumeData - Volume analysis indicator",
    ...
    def get_config(self) -> Dict[str, Any]:
        return {"name": "VolumeData", "category": self.CATEGORY, 
