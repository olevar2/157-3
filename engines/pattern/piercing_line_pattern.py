...
class PiercingLinePattern(IndicatorBase):
        ...
        for i in range(1, len(candles)):
            ...
            if self._is_valid_piercing_line(candle1, candle2):
                ...
                strength = self._calculate_pattern_strength(
                    candle1, candle2, candles, i
                )
                confidence = self._calculate_confidence(
                    candle1, candle2, pierce_percentage
                )
    ...
    def _calculate_pierce_percentage(
        self, candle1: CandleData, candle2: CandleData
    ) -> float:
    ...
    def _calculate_pattern_strength(
        ...
        # Body size bonus
        avg_range = np.mean(
            [c.high - c.low for c in candles[max(0, position - 10) : position]]
        )
        if avg_range > 0:
    ...
    def _calculate_confidence(
        self, candle1: CandleData, candle2: CandleData, pierce_percentage: float
    ) -> float:
